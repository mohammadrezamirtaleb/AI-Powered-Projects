...
| **Traffic Rules** | Right-Hand Traffic (RHT) | Southbound uses `x < cx`, Northbound uses `x > cx` |
| **Roundabout Radii** | `30 / 48 / 74 / 102 px` | Central island, inner ring center, outer ring center, curb limit |
| **Circulation** | Counter-Clockwise (CCW) | Strictly enforced CCW flow (`N -> W -> S -> E`) |
| **Signalized 4-Way** | Adaptive Phase Ring | `NS_GREEN` -> `NS_YELLOW` -> `ALL_RED` -> `EW_GREEN` -> `EW_YELLOW` -> `ALL_RED` |
...
The agent receives a 3-frame stacked observation vector (3 × 52 = 156 dimensions) capturing dense spatial, kinematic, and semantic environmental context:

| Component | Dims / Frame | Total Dims | Description |
| :--- | :---: | :---: | :--- |
| **LiDAR Rays** | 18 | 54 | 9 forward-facing rays (±60° FOV): range [0, 1] and relative closing velocity |
...
| :---: | :--- | :---: | :--- |
| `0` | **COAST** | `0.0 m/s²` | Maintain velocity, zero throttle / brake resistance |
| `1` | **ACCEL_MILD** | `+1.2 m/s²` | Smooth cruising adjustment, low-speed gap following |
| `2` | **ACCEL_FULL** | `+2.8 m/s²` | Merging into roundabout gaps, clearing green lights |
| `3` | **BRAKE_MILD** | `-1.8 m/s²` | Approaching stop lines, gradual deceleration |
| `4` | **BRAKE_HARD** | `-4.5 m/s²` | Emergency stopping, pedestrian yield, collision avoidance |
...
2. **Training Mode (`Key 2`)**: Blends ε-greedy DQN exploration with expert intervention and active shield monitoring.
3. **Master Mode (`Key 3`)**: Pure neural policy inference with active safety shield verification.

> [!IMPORTANT]
> **Safety Shield Guarantees:**
> - **TTC Clamp**: Intercepts acceleration actions if Time-To-Collision (TTC) falls below safety margins (`< 1.0s`).
...
| :--- | :---: | :---: | :--- | :--- |
| **Ambulance** | `44 px` | `5.5 px/frame` | White body, red cross, dual red/blue lightbar | Rapid medical transit |
| **Fire Truck** | `62 px` | `5.2 px/frame` | Heavy red chassis, amber roof deck, strobe | Heavy rescue response |
| **Police Cruiser** | `40 px` | `6.0 px/frame` | Midnight navy, high-frequency alternating flashers | Fast tactical intercept |

- **Signal Preemption**: Approaching EMS units detect opposing green phases and trigger an immediate safe yellow -> all-red -> emergency green cycle with an 8-second cooldown.
- **Civilian Lateral Evasion**: Surrounding civilian vehicles detect siren acoustics (`distance < SIREN_RADIUS`) and shift laterally by `+12 px` toward the road shoulder while reducing speed.
...
- **Weather Physics**: Dynamic switching between `CLEAR`, `RAIN`, and `STORM` conditions, dynamically reducing road friction (friction coefficient `μ ∈ [0.45, 1.0]`) and lengthening braking distances.
...
| <kbd>2</kbd> | **Training Mode** | Run active ε-greedy DQN training loop with experience collection |
| <kbd>3</kbd> | **Master Mode** | Run inference using loaded pretrained neural weights |
| <kbd>W</kbd> | **Weather Toggle** | Cycle through Clear -> Rain -> Storm |
| <kbd>N</kbd> | **Day / Night Cycle** | Cycle through Day -> Sunset -> Night lighting |
...
│   ├── 📄 test_city_network.py            # Dual-node & roundabout network tests
│   ├── 📄 test_collision.py               # SAT polygon collision tests
│   ├── 📄 test_replay_buffer.py           # Prioritized Experience Replay tests
│   ├── 📄 test_resize.py                  # Viewport resize & coordinate scaling tests
│   ├── 📄 test_reward.py                  # Multi-objective reward shaping tests
│   └── 📄 test_situation_policy.py       # 156-D observation & situation gate tests
